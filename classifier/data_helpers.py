...
def create_vec_for_prod_data(text,seq_len,vocabulary):
    #x_train, y_train, vocabulary, vocabulary_inv,labels_dict,w2v_for_vocab = load_data()
    ...
    padded_sentences = []
    for sent in [x_text]:
        if len(sent) < seq_len: #x_train.shape[1]:
            #sent = sent + ["<PAD/>"]*(x_train.shape[1]-len(sent))
...
def load_data_and_labels():
    ...
    mysql_cn= MySQLdb.connect(host='127.0.0.1',
                ...
                db='maya_audiences')
    #df = pd.read_sql('select * from tweets_clean ORDER BY RAND();', con=mysql_cn)
    df = pd.read_sql('select * from tweets_clean ORDER BY RAND();', con=mysql_cn)
    '''
    df=None
