...
import data_helpers
from text_cnn_with_embeddings import TextCNN
import cleaner
import ast
import data_helpers
#import MySQLdb
...
def find_subcategory(first_category,tweet):
    ...
    tweet = [cleaner_tweets.clean(' '.join(tknzr.tokenize(tweet.decode('utf-8','ignore')))).replace('<hashtag>', '').replace('<allcaps>', '')]
...
def classify_tweets():
    ...
    with tf.Graph().as_default():
        ...
        df = pd.read_sql('select * from tweets_test;', con=mysql_cn)
        '''
        df = pd.read_csv(os.path.join(os.path.curdir,'resources','july_25.csv'),header=None, error_bad_lines=False)
        ...
        with open(os.path.join(os.path.curdir,'results','real_test.csv'), 'wb') as csvfile:
                ...
                if type(eachRow[1][4]) == str:
                    ...
                    for k in labels_dict.keys():
                        if labels_dict[k][first_predict] == 1:
                            ...
                            #spamwriter.writerow([eachRow[1]['text'],eachRow[1]['label'],k])
            ...
            #                print mapper_dict['Desc'][k]
                    #print scores
                    #break
                '''
                except Exception,e:
                    logger.error(eachRow[1][4])
                    logger.error(str(e))
                '''
